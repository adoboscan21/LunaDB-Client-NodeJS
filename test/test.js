...
async function runTest(testName, testFn) {
  process.stdout.write(`⏳ Testing: ${testName.padEnd(55, ".")} `);
  try {
...
async function main() {
  ...
  // ---------------------------------------------------------
  // PHASE 6: Server-Side Mass Mutations (Where)
  // ---------------------------------------------------------
  await runTest("14. Mass Mutations (Update Where & Delete Where)", async () => {
    // Preparar datos
    await client.collectionItemSetMany(testCol, [
      { _id: "m_1", status: "pending", amount: 100 },
      { _id: "m_2", status: "pending", amount: 200 },
      { _id: "m_3", status: "shipped", amount: 300 }
    ]);

    // Update Where: Actualizar a 'processed' todos los que estén 'pending'
    await client.collectionUpdateWhere(
      testCol,
      { filter: { field: "status", op: "=", value: "pending" } },
      { status: "processed" }
    );

    const m1 = await client.collectionItemGet(testCol, "m_1");
    const m3 = await client.collectionItemGet(testCol, "m_3");
    assert.strictEqual(m1.value.status, "processed", "El item m_1 debió actualizarse a processed");
    assert.strictEqual(m3.value.status, "shipped", "El item m_3 no debió verse afectado");

    // Delete Where: Eliminar todos los 'processed'
    await client.collectionDeleteWhere(testCol, { filter: { field: "status", op: "=", value: "processed" } });

    const m1_after = await client.collectionItemGet(testCol, "m_1");
    const m3_after = await client.collectionItemGet(testCol, "m_3");
    assert.strictEqual(m1_after.found, false, "El item m_1 debió ser eliminado");
    assert.strictEqual(m3_after.found, true, "El item m_3 debió sobrevivir al borrado masivo");
  });

  // ---------------------------------------------------------
  // PHASE 7: Query Engine
  // ---------------------------------------------------------
  await runTest("15. Queries (Filters, Limit, OrderBy)", async () => {
  ...
  await runTest("16. Queries (Aggregations and Group By)", async () => {
    const results = await client.collectionQuery(testCol, {
      group_by: ["cat"],
      aggregations: {
  ...
  await runTest("17. Queries (Lookups / Joins)", async () => {
  ...
  // ---------------------------------------------------------
  // PHASE 8: ACID Transactions (Stateful)
  // ---------------------------------------------------------
  await runTest("18. Transactions: Commit", async () => {
  ...
  await runTest("19. Transactions: Rollback", async () => {
    const tx = await client.begin();
  ...
  await runTest("20. Transactions: Update/Delete Where", async () => {
    await client.collectionItemSetMany(testCol, [
      { _id: "tx_m_1", type: "temp", v: 1 },
      { _id: "tx_m_2", type: "temp", v: 2 }
    ]);

    const tx = await client.begin();

    // Update Where dentro de TX
    await tx.collectionUpdateWhere(testCol, { filter: { field: "type", op: "=", value: "temp" } }, { updated: true });

    // Delete Where dentro de TX
    await tx.collectionDeleteWhere(testCol, { filter: { field: "_id", op: "=", value: "tx_m_1" } });

    await tx.commit();

    const r1 = await client.collectionItemGet(testCol, "tx_m_1");
    const r2 = await client.collectionItemGet(testCol, "tx_m_2");
    assert.strictEqual(r1.found, false, "El registro debió ser eliminado masivamente en la TX");
    assert.strictEqual(r2.value.updated, true, "El registro debió ser actualizado masivamente en la TX");
  });

  // ---------------------------------------------------------
  // PHASE 9: Cleanup and Close
  // ---------------------------------------------------------
  await runTest("21. Delete Collections (Drop)", async () => {
